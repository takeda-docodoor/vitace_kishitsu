...
<head>
  ...
  <title>氣質診断管理システム | 気質認定証明情報</title><!-- 20171002 -->
...
function navi_win( Masterid ) {
        ...
        if (w != null) {
                ...
                w.location.href = './kishitsucert_edit.php?mstact=mstedit&masterid=' + Masterid;
        }
}
</script>
<script language="JavaScript" type="text/javascript">
function navi_win1( Masterid ) {
        ...
        if (w != null) {
                if (w.opener == null) {w.opener = self;}
                w.location.href = './kishitsucert_edit.php?mstact=mstnew&masterid=' + Masterid;
        }
}
</script>
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview active">
          ...
          <ul class="treeview-menu">
            ...
            <li><a href="kishitsucert_list.php"><i class="fa fa-circle-o"></i>認定証情報一覧</a></li>
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="kievent_list.php"><i class="fa fa-circle-o"></i>イベント一覧</a></li>
          </ul>
        </li>
        <li class="treeview">
          <a href="#">
            ...
            <div class="box-body">
              <table class="table table-bordered table-hover">
                ...
                <tr>
                  ...
                  <th class="huntsys1">国</th>
                  <th class="huntsys1">取得年</th>
                  <th class="huntsys1">認定番号</th>
                  <th class="huntsys1">取得級</th>
                  <th class="huntsys1">認定証</th>
                  <th class="huntsys1">認定送付</th>
			  ...
			  <?php
				...
				$sql101 = '';
				$result101 = '';
				$where101 .= "ksc_memberid > '" . '0' . "' and ";
				$where101 .= '1 = 1';
				$sql101 = "select * from kishitsucert where $where101 order by ksc_memberid, kishitsucert_id";
				$result101 = mysqli_query( $link1, $sql101 ) or die('query error284' . mysqli_error($link1));

			  ?>
			  <?php $m = 0; while ( $kishitsucert = mysqli_fetch_array( $result101 ) ) { ?>
				  <?php
					$where103 = '';
					$sql103 = '';
					$result103 = '';
					$sql103 = "select * from member where member_id = '" . $kishitsucert['ksc_memberid'] . "'";
					$result103 = mysqli_query( $link1, $sql103 );
					$member = mysqli_fetch_array( $result103 );
				  ?>
				<tr>
				  <td class="huntsys1"><?php echo $member['mbr_kanji'] ?></td>
				  <td class="huntsys1"><?php echo $country_tbl[$kishitsucert['ksc_country']] ?></td>
				  <td class="huntsys1"><?php echo $kishitsucert['ksc_getyear'] ?></td>
				  <td class="huntsys1"><?php echo $kishitsucert['ksc_certno'] ?></td>
				  <td class="huntsys1"><?php echo $stepclass_tbl[$kishitsucert['ksc_stepclass']] ?></td>
				  <?php if ($kishitsucert['ksc_certificate'] != '') { ?>
				    <td class="huntsys1"><a href="../../certimg/<?php echo $kishitsucert['ksc_certificate'] ?>" target="_blank">表示</a></td>
                  <?php } else { ?>
				    <td class="huntsys1">未</td>
                  <?php } ?>
				  <td class="huntsys1"><?php echo $certsend_tbl[$kishitsucert['ksc_certsend']] ?></td>
				  <td class="huntsys1"><a href="javascript:navi_win( '<?php echo $kishitsucert['kishitsucert_id'] ?>' )">編集</a></td>
				  <td class="huntsys1"><a href="kishitsucert_list.php?kishitsucert_id=<?php echo $kishitsucert['kishitsucert_id'] ?>&action=kishitsucertdel" alt="削除" title="データを削除します。" onClick="return confirm( 'データを削除します。よろしいですか？' )">削除</a></td>
				</tr>
              <?php $m ++; } ?>
              ...
              <div class="box-footer">
				  <a href="javascript:navi_win1( '<?php echo $_SESSION['member_id']  ?>' )"><button class="btn btn-primary">新規追加</button></a>
              </div>
            </div>
            <!-- /.box-body -->
          </div>
